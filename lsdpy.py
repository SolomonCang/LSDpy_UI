...
# Save LSD output summary with detailed statistics and parameters
# Determine lsdout filename based on params settings
if (params.fSaveLSDOut != 0):
    if (params.outLSDOutName == 'auto' or params.outLSDOutName == ''):
        lsdoutName = outputProfName.replace('.dat', '_lsdout.txt')
    else:
        lsdoutName = params.outLSDOutName

    lsdpFunc.saveLSDOut(lsdoutName,
                        spectrumFile=params.inObs,
                        maskFile=params.inMask,
                        profileFile=outputProfName,
                        params=params,
                        obs=obs,
                        maskObj=mask,
                        prof=prof,
                        ...
                        modelOptions={
                            ...
                            params.saturationCorrection,
                            'telluricFiltering': params.telluricFiltering,
                            'lineFiltering': params.lineFiltering
                        })
else:
    print('LSD output summary (lsdout) disabled in inlsd.dat')
